...
.NOTES
    In order for this script to function the deployed resources must have a tag _deployed_by_cafm with a value of true and Policy resources must have metadata property
    named _deployed_by_cafm with a value of True. These tags and metadata are included in the automation, but if they are subsequently removed, there may be orphaned
    resources after this script executes.

    The Role Assignments associated with Policy assignment identities and including _deployed_by_cafm in the description field will also be deleted.
...
.EXAMPLE
    ./Start-CAFMCleanup.ps1 -ReportOnly
    # generate a list of the resource IDs which would be deleted by this script

.EXAMPLE
    ./Start-CAFMCleanup.ps1 -WhatIf
    # show output of what would happen if deletes executed

.EXAMPLE
    ./Start-CAFMCleanup.ps1 -Force
...
$query = "Resources | where type in~ ('Microsoft.Insights/metricAlerts','Microsoft.Insights/activityLogAlerts', 'Microsoft.Insights/scheduledQueryRules') and tags['_deployed_by_cafm'] =~ 'True' | project id"
...
Write-Host "Found '$($alertResourceIds.Count)' metric, activity log and log alerts with tag '_deployed_by_cafm=True' to be deleted."

# get resource group to delete
$query = "ResourceContainers | where type =~ 'microsoft.resources/subscriptions/resourcegroups' and tags['_deployed_by_cafm'] =~ 'True' | project id"
...
Write-Host "Found '$($resourceGroupIds.Count)' resource groups with tag '_deployed_by_cafm=True' to be deleted."

# get policy assignments to delete
$query = "policyresources | where type =~ 'microsoft.authorization/policyAssignments' | project name,metadata=parse_json(properties.metadata),type,identity,id | where metadata._deployed_by_cafm =~ 'true'"
...
Write-Host "Found '$($policyAssignmentIds.Count)' policy assignments with metadata '_deployed_by_cafm=True' to be deleted."

# get policy set definitions to delete
$query = "policyresources | where type =~ 'microsoft.authorization/policysetdefinitions' | project name,metadata=parse_json(properties.metadata),type,id | where metadata._deployed_by_cafm =~ 'true' | project id"
...
Write-Host "Found '$($policySetDefinitionIds.Count)' policy set definitions with metadata '_deployed_by_cafm=True' to be deleted."

# get policy definitions to delete
$query = "policyresources | where type =~ 'microsoft.authorization/policyDefinitions' | project name,metadata=parse_json(properties.metadata),type,id | where metadata._deployed_by_cafm =~ 'true' | project id"
...
Write-Host "Found '$($policyDefinitionIds.Count)' policy definitions with metadata '_deployed_by_cafm=True' to be deleted."

# get role assignments to delete
$query = "authorizationresources | where type =~ 'microsoft.authorization/roleassignments' and properties.description == '_deployed_by_cafm' | project roleDefinitionId = properties.roleDefinitionId, objectId = properties.principalId, scope = properties.scope, id"
...
Write-Host "Found '$($roleAssignments.Count)' role assignment with description '_deployed_by_cafm' to be deleted."
